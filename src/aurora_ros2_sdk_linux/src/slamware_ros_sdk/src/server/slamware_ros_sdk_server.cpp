...
namespace slamware_ros_sdk {
    ...
    bool SlamwareRosSdkServer::init_(std::string& /*errMsg*/)
    {
        ...
        // init all workers
        {
            ...
            if (params_.getParameter<bool>("stereo_camera_info_enable")) {
                // ServerStereoCameraInfoWorker: Classic Aurora returns -7 (not supported); use aurora_sdk_bridge instead
                auto svrWk = std::make_shared<ServerStereoCameraInfoWorker>(this, "StereoCameraInfo", sfConvFloatSecToChronoMs_(params_.getParameter<float>("stereo_camera_info_pub_period")));
                serverWorkers_.push_back(svrWk);
            }
    ...
    void RawImageListener::onRawCamImageData(uint64_t timestamp_ns, const RemoteImageRef& left, const RemoteImageRef& right){
        ...
        if (rawFrameL.empty() || rawFrameR.empty())
            return;
        if (rawFrameL.channels() != 1 && rawFrameL.channels() != 3) return;
        if (rawFrameR.channels() != 1 && rawFrameR.channels() != 3) return;

        rclcpp::Time pair_stamp = server_->get_clock()->now();
        ...
        header_pair.frame_id = CameraFrameLeftId;
        cv_bridge::CvImage img_bridge_left(header_pair, sensor_msgs::image_encodings::RGB8, rawFrameL);
        pubLeftRawImage_->publish(*img_bridge_left.toImageMsg());

        if (rawFrameR.channels() == 3)
            cv::cvtColor(rawFrameR, rawFrameR, cv::COLOR_BGR2RGB);
        else
            cv::cvtColor(rawFrameR, rawFrameR, cv::COLOR_GRAY2RGB);
        header_pair.frame_id = CameraFrameRightId;
        cv_bridge::CvImage img_bridge_right(header_pair, sensor_msgs::image_encodings::RGB8, rawFrameR);
        pubRightRawImage_->publish(*img_bridge_right.toImageMsg());
    }
