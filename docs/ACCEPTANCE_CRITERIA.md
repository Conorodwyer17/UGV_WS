# Acceptance Criteria for Tyre Inspection Mission

This document defines acceptance criteria for validating the tyre inspection mission on a real Aurora robot with physical cars.
...
| 5 | Tyre inspection | Robot visits 4 tyres in distance order (nearest first, then 2nd, 3rd, 4th nearest); stops ~0.5 m in front of each |
| 6 | Photo capture | Photos captured for each tyre; VERIFY_CAPTURE succeeds 4 times |
...
- Planned tyre fallback used when no wheel detection within tire_search_timeout_s (90 s)
